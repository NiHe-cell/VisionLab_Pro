#include "OnnxRuntimeCudaEngine.h"
...
std::string cudaFailure(const std::string& message, int deviceId)
{
    std::string error = message;
    if (error.find("CUDA") == std::string::npos && error.find("cuda") == std::string::npos)
        error = "CUDA EP failed: " + error;
    error += " (device " + std::to_string(deviceId) + ")";
    return error;
}

} // namespace

struct OnnxRuntimeCudaEngine::Impl
{
...
OnnxRuntimeCudaEngine::OnnxRuntimeCudaEngine()
    : m_impl(std::make_unique<Impl>())
{
}

OnnxRuntimeCudaEngine::~OnnxRuntimeCudaEngine() = default;

bool OnnxRuntimeCudaEngine::initialize(const ModelConfig& config)
{
    m_impl->reset();
    m_impl->deviceId = config.deviceId;

    if (config.backend != InferenceBackend::OnnxRuntimeCuda)
    {
        m_impl->error = "OnnxRuntimeCudaEngine only supports InferenceBackend::OnnxRuntimeCuda";
        return false;
    }
    if (config.precision != InferencePrecision::Fp32)
    {
        m_impl->error = "OnnxRuntimeCudaEngine only supports InferencePrecision::Fp32";
        return false;
    }
    ...
    try
    {
        m_impl->env = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "visionlab-cuda");
        Ort::SessionOptions options;
        options.SetIntraOpNumThreads(1);
        options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_BASIC);

        OrtCUDAProviderOptions cudaOptions;
        cudaOptions.device_id = config.deviceId;
        ...
        {
            m_impl->error = "OnnxRuntimeCudaEngine expects exactly one model input";
            m_impl->reset();
            return false;
        }
        ...
        {
            m_impl->error = "OnnxRuntimeCudaEngine expects float32 input";
            m_impl->reset();
            return false;
        }
    ...
    {
        m_impl->reset();
        m_impl->error = cudaFailure(ex.what(), config.deviceId);
        return false;
    }
}

bool OnnxRuntimeCudaEngine::isReady() const
{
    return m_impl->ready;
}

std::string OnnxRuntimeCudaEngine::lastError() const
{
    return m_impl->error;
}

std::string OnnxRuntimeCudaEngine::backendId() const
{
    return "onnxruntime-cuda";
}

DeviceInfo OnnxRuntimeCudaEngine::device() const
{
    DeviceInfo info;
    info.name = "CUDA";
...
TensorMetadata OnnxRuntimeCudaEngine::inputMetadata() const
{
    return m_impl->inputMeta;
}

std::vector<TensorMetadata> OnnxRuntimeCudaEngine::outputMetadata() const
{
    return m_impl->outputMeta;
}

InferResult OnnxRuntimeCudaEngine::infer(const TensorView& input)
{
    InferResult result;
    ...
    {
        result.ok = false;
        result.error = cudaFailure(ex.what(), m_impl->deviceId);
        result.outputs.clear();
        return result;
    }
}

void OnnxRuntimeCudaEngine::warmup(int iterations)
{
